...
def claim_daily():
    ...
    with write_lock:
        ...
        if not user:
            return jsonify({'error': 'User not found'}), 404
        now = int(time.time())
        last_claim = user[7] or 0
        elapsed = now - last_claim
        if elapsed < DAILY_COOLDOWN:
            remaining = DAILY_COOLDOWN - elapsed
            hours = (remaining + 3599) // 3600
            return jsonify({'error': f'Жди {hours} ч'}), 400
        new_bal = user[1] + DAILY_REWARD
        update_user(uid, balance=new_bal, daily_claimed=now)
        return jsonify({'success': True, 'reward': DAILY_REWARD, 'new_balance': new_bal})
