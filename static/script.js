...
// Красивый вывод шанса (доля -> строка %)
function formatChance(p) {
    const pct = p * 100;
    if (pct >= 1) return (Math.round(pct * 100) / 100) + '%';
    let s = pct.toFixed(7).replace(/0+$/, '').replace(/\.$/, '');
    return s + '%';
}
...
function showTape(type, mode = 'preview') {
    ...
    if (mode === 'preview') {
        ...
        // Шансы редкостей
        const rarityCfg = CASE_RARITY[type];
        if (rarityCfg && rarityCfg.chances) {
            const chancesBox = document.createElement('div');
            chancesBox.className = 'pv-chances';
            for (const key of ['common', 'rare', 'epic', 'legendary', 'jackpot']) {
                const chance = rarityCfg.chances[key];
                if (chance === undefined) continue;
                const meta = RARITY_META[key];
                const chip = document.createElement('div');
                chip.className = 'pv-chance-chip ' + meta.cls;
                chip.innerHTML = `<span class="pv-chance-label">${meta.label}</span><span class="pv-chance-val">${formatChance(chance)}</span>`;
                chancesBox.appendChild(chip);
            }
            scroll.appendChild(chancesBox);
        }
...
function showBotBattlePreview(case_type) {
    ...
    const levelName = level ? level.name : getCaseName(case_type);
    ...
    overlay.innerHTML = `
        ...
        <div style="display:flex; gap:40px; align-items:center; margin-bottom:16px;">
            ...
            <div style="font-size:36px; font-weight:900; color:#f87171;"><img src="assets/vs.png" class="battle-vs-img" alt="" onerror="this.remove()"></div>
            <div style="text-align:center;">
                <div style="font-size:40px;">🤖</div>
        ...
        <button onclick="this.closest('#botBattlePreviewOverlay').remove(); startBotBattle('${case_type}');" style="padding:14px 40px; border:none; border-radius:14px; background:linear-gradient(135deg, #b39df7, #8b7cf0); color:#fff; font-size:18px; font-weight:700; cursor:pointer; box-shadow:0 4px 20px rgba(139,124,240,0.25);">
            <img src="assets/icon_battle.png" class="ic" style="width:22px;height:22px;vertical-align:-5px;" alt="⚔️" onerror="this.outerHTML='⚔️'"> НАЧАТЬ БИТВУ
        </button>
...
function showBotRouletteAnimationWithResult(data, case_type) {
    ...
    title.textContent = `⚔️ Битва: «${getCaseName(case_type)}»`;
    overlay.appendChild(title);
    ...
    vsDiv.style.cssText = 'text-align:center; flex-shrink:0; padding:2px 0; line-height:0;';
    vsDiv.innerHTML = '<img src="assets/vs.png" style="width:72px;height:72px;object-fit:contain;filter:drop-shadow(0 0 20px rgba(248,113,113,0.55));" alt="⚔️ VS" onerror="this.outerHTML=\'<div style=&quot;font-size:28px;font-weight:900;color:#f87171;text-shadow:0 0 30px rgba(248,113,113,0.3);&quot;>⚔️ VS</div>\'">';
        ...
        apiRequest('/crash_status', {}, 1, 4000).then(status => {
            ...
            if (status.phase === 'preview') {
                ...
                if (crash.hasBet) {
                    ...
                    loadBalance();
                    loadCrashStats();
                    if (lostBet > 0 && prevPhase !== 'crashed' && prevPhase !== 'crash') {
                        createResultOverlay({
                            ...
                            title: 'КРАШ!',
                            amount: lostBet,
                            subtitle: `Ракета упала на x${(status.multiplier || 1).toFixed(2)}. Ставка потеряна.`,
                            buttons: [
                                { label: '👍 ПОНЯТНО', cls: 'btn-primary', onClick: () => { const o = document.getElementById('crashResultOverlay'); if (o) o.remove(); } }
                            ]
                        });
                    }
                }
...
function showMinesResult(isWin, amount, multiplier = 1) {
    ...
    createResultOverlay({
        ...
        buttons: [
            ...
            { label: '🔙 К ИГРАМ', cls: 'btn-ghost', onClick: () => { closeMinesResult(); showGames(); } }
        ],
...
function showGameUpgradeResult(result, message, newBalance) {
    ...
    createResultOverlay({
        ...
        buttons: [
            ...
            { label: '🔙 К ИГРАМ', cls: 'btn-ghost', onClick: () => { const o = document.getElementById('gameUpgradeResultOverlay'); if (o) o.remove(); showGames(); } }
        ],
        confetti: isWin,
        confettiCount: 30
    });
